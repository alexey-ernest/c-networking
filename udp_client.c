/* a simple udp client using the similar workflow as a tcp client (connect() and send())
   rather than sendto() and recvfrom() workflow */
...
int main(int argc, char const *argv[])
{
	...
	while (1) {
		...
		if (FD_ISSET(0, &reads)) {
			...
			int bytes_sent = send(socket_peer, read, strlen(read), 0); /* ignore send errors becasue we'll handle them on recv call shortly */
			printf("Sent %d bytes.\n", bytes_sent);
		}
	}
